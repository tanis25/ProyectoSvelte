...
<div class="container">
			...
			<div class="filtros-grid">
				...
				<div class="fechas-row">
					...
					<div class="botones-filtros">
						...
						<button type="button" class="btn-filtro gray" on:click={limpiar}>
							Limpiar
						</button>
					</div>
				</div>
	...
	{#if data.totalPages > 0}
		...
		<div class="paginacion">
			...
			{#each Array.from({ length: maxPages }, (_, i) => i + 1) as p}
				{#if p === 1 || p === maxPages || (p >= current - rango && p <= current + rango)}
					<a href="?page={p}&search={search}&desde={desde}&hasta={hasta}" class={p === current ? 'activo' : ''}>
						{p}
					</a>
				{:else if p === current - rango - 1 || p === current + rango + 1}
					<span>...</span>
				{/if}
			{/each}
